...
def generate_reward(current_floor: int, is_healer: bool):
    ...
    if reward_type == "weapon":
        weapon_names = ["Mace", "Sword", "Whip", "Greatsword", "Lance", "Spear", "Axe"]
        healer_names = ["Wand", "Staff", "Spellbook", "Spell"]
        random_mod = random.choice(modifier_name)
        
        if is_healer:
            name = random.choice(healer_names)
            item = Weapon(
...
def get_combat_order(match: dict, cur):
    ...
    return combat_order, active_player_ids
...
def advance_turn_system(match_id: int, cur):
    ...
    combat_order, active_player_ids = get_combat_order(match, cur)
    
    if not active_player_ids or not any(e[0] == "enemy" for e in combat_order):
        return

    current_index = match.get("turn_index") or 0
    ...
    try:
        ...
        cur.execute("""
            INSERT INTO public.players(
                player_name, player_class, base_max_health, max_health, current_health, 
                base_damage, damage, base_healing_capacity, healing_capacity, base_defence, defence
            ) VALUES (
                %(name)s, %(class)s, %(base_max_health)s, %(max_health)s, %(current_health)s, 
                %(base_damage)s, %(damage)s, %(base_healing_capacity)s, %(healing_capacity)s, 
                %(base_defence)s, %(defence)s
            ) RETURNING *;
        """, params)
...
def get_reward_choices(player_id: int):
    ...
    try:
        ...
        cur.execute("SELECT player_class FROM players WHERE id = %s", (player_id,))
        player_row = cur.fetchone()
...
def select_reward(player_id: int, req: SelectionRequest):
    ...
    try:
        ...
        cur.execute("INSERT INTO items (name, sprite_path) VALUES (%s, %s) RETURNING id", (choice["name"], choice["sprite_path"]))
...
def join_match(player_id: int):
    ...
    try:
        ...
        if match:
            slot_to_fill = None
            if match["player_1_id"] is None:
                slot_to_fill = "player_1_id"
            elif match["player_2_id"] is None:
                slot_to_fill = "player_2_id"
            elif match["player_3_id"] is None:
                slot_to_fill = "player_3_id"
            elif match["player_4_id"] is None:
                slot_to_fill = "player_4_id"
...
def get_matches():
    ...
    try:
        cur.execute("SELECT * FROM matches WHERE status = 'waiting'")
...
def attack_enemy(match_id: int, payload: AttackPayload = Body(...)):
    ...
    try:
        ...
        combat_order, active_player_ids = get_combat_order(match, cur)
        current_turn_index = match.get("turn_index") or 0
...
def heal_player(match_id: int, payload: HealPayload = Body(...)):
    ...
    try:
        ...
        cur.execute("UPDATE players SET current_health = %s WHERE id = %s", (new_hp, payload.target_player_id))
